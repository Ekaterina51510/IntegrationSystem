// Клиент для численного интегрирования методом прямоугольников
// Клиент подключается к серверу, получает отрезок интегрирования и шаг,
// вычисляет интеграл функции 1 / ln(x) методом средних прямоугольников
// и возвращает результат на сервер.
//Метод прямоугольников :
//∫f(x)dx ≈ h* Σ f(x_i + h / 2), где h - шаг интегрирования
#include <iostream>
#include <sstream>
#include <fstream>
#ifdef _WIN32
...
#endif
#include "calculator.h"// Заголовочный файл с реализацией метода прямоугольников
using namespace std;
...
int main()
{
    ...
    ofstream logfile("client.log");// Создание лог-файла для записи хода выполнения
    if (!logfile.is_open()) 
    {
        cout << "Ошибка создания лог-файла!" << endl;
        return 1;
    }
    cout << "ИНТЕГРАЦИОННЫЙ КЛИЕНТ(метод прямоугольников)\n\n";
    logfile << "ИНТЕГРАЦИОННЫЙ КЛИЕНТ(метод прямоугольников)" << endl;
    ...
    // Определение количества ядер процессора для отправки серверу
    int my_cores = IntegrationCalculator::getAvailableCores();
    ...
    logfile << "Шаг: " << h << endl;
    cout << "\nВычисляю...\n";
    logfile << "Вычисляю..." << endl;
    double result = IntegrationCalculator::integrate(a, b, h);// Вычисление интеграла методом прямоугольников
    ...
    closesocket(clientSocket);// Закрытие соединения и завершение работы
#ifdef _WIN32
    WSACleanup();
#endif
    cout << "Клиент завершил работу. Нажмите Enter...\n";
    logfile << "Клиент завершил работу. Нажмите Enter..." << endl;
    logfile.close();
    cin.get();
}